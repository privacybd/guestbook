...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="privacybd/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/privacybd/guestbook.git'
            }
        }
        stage('Build') {
            steps {
    ...
    post { 
        always { 
            emailext (attachLog: true, body: '본문', compressLog: true
                    , recipientProviders: [buildUser()], subject: '제목', to: 'gonetworker75@gmail.com')
